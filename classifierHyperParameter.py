...
postProcWinSpace = np.arange(11 , 11 + nHyper)
...
dataPath = os.path.join(currentPath ,"performanceEvaluationDataVote","sub{}".format(sub))
...
timesAll , runScores ,  seizureTimes , badsAll = myEval.runExp(sub , nRuns , scoreType = scoreType , \
 classifier = classifier , autorejcetFactor = autorejcetFactor  , transferLearning =  tl , others = 2\
                ,selfFrac = 1 , tlFrac = 0.5 , preIctal =  preIctal , currentPath = currentPath )
...
for postProcessWindow in postProcWinSpace:
    ...
    "sub_{}sop_{}_sph_{}_postProc_{:.2f}_{:.2f}_TL_{}_score_{}_sustain_{}_{}_autoreject_{}_Vote.pkl".format(\
            ...
            for clfInd in range(len(classifiers)):
                ...
                for kRun in range(nRuns):
                    ...
                    spec = 1 - numFalse * (sop + sph)/interIctalHours
